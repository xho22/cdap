...
export default class SpotlightSearch extends Component {
  ...
  handleCloseSearch(event) {
    ...
    if (event && event.keyCode !== 27 && event.target === this.spotlightSearch) { return; }
    let app = document.getElementById('app-container');
  ...
  handleRenderResult() {
    if (this.state.searchQuery.length === 0) { return null; }
  ...
  handleKeyPress(event) {
    ...
    if (event.keyCode === keyMap.arrowUp) {
      ...
      let limit = this.state.searchResults.total > VIEW_RESULT_LIMIT ?
        VIEW_RESULT_LIMIT : this.state.searchResults.total - 1;
